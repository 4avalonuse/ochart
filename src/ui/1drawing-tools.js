...
// ============================================================
// Ferramentas de Desenho completas para o ochart (versão com
// expansão do Fibonacci + painel de níveis em menu expansível)
...
//   retângulo, fibonacci (retrações + expansões), texto (placeholder),
...
export class DrawingTools {
  constructor(chartEngine, options = {}) {
    ...
    // Fibo: níveis padrão (retração) + níveis de expansão
  ...
  _renderToolbar() {
    ...
    el.innerHTML = `
      ...
      <div class="toolbar-section">
        ...
        <!-- Menu expansível para não ocupar espaço -->
    ...
    // Popular níveis de fibo (retração + expansão)
  ...
  _renderFibLevels(container) {
    ...
    const block = (title, levels, checked = true) => `
    ...
    container.innerHTML =
      block('Retração', this.fibLevels, true) +
      block('Expansão', this.fibExtLevels, true);
  }
      ...
      switch (this.currentTool.id) {
        ...
        case 'fib':
          this._addDrawing({
            ...
            x1:this.startPoint.x, y1:this.startPoint.y,
            x2:this.endPoint.x,   y2:this.endPoint.y,
            // níveis serão lidos do painel (retração + expansão)
...
// dentro de _convertForEngine(drawings)
if (d.type === 'fib') {
  // Níveis do painel; se vazio, usa retração + expansão padrão
  let lvls = this._readFibLevelsChecked();
  if (!Array.isArray(lvls) || lvls.length === 0) {
    lvls = this.fibLevels.concat(this.fibExtLevels);
  }

  // span horizontal do arrasto
  const xLeft  = Math.min(d.x1, d.x2);
  const xRight = Math.max(d.x1, d.x2);

  // normaliza topo/fundo independente da direção
  ...
  const isLog  = this.engine?.chart?.options?.scales?.y?.type === 'logarithmic';

  for (const l of lvls) {
    // baixa:  y = high - range*l   |   alta: y = low + range*l
    let y = isDown ? (yHigh - range * l) : (yLow + range * l);
  ...
  // === SALVAR: tenta servidor; se falhar, baixa arquivo ===
  async _saveJSON() {
