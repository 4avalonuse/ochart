...
export async function sync(engine, tf, currentScale, currentType, options = {}) {
  ...
  pushLog({
    ...
    msg:'sync_start',
    ts:Date.now(),
    data:{ tf, scale:currentScale, forceRefresh }
  });

  try{
    const payload = await fetchSeries(tf, currentScale, { forceRefresh });
    ...
    pushLog({
      ...
      data:{
        ...
        staleFallback:meta?.staleFallback ?? false,
    ...
    pushLog({
      ...
      msg:'sync_fail',
      ts:Date.now(),
      data:{ error:String(e), forceRefresh }
    });
